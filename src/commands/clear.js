...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (interaction.guild == null)
            return interaction.editReply({ content: "You can use this command only on servers!", ephemeral: true });
        if (!interaction.channel.permissionsFor(interaction.member).has('MANAGE_MESSAGES'))
            return interaction.editReply({ content: "You do not have enough permissions to do this.", ephemeral: true });

        var amount = interaction.options.getInteger('amount');
        if (!amount) amount = 100;
        const user = interaction.options.getUser('target');

        if (amount < 1 || amount > 100) return interaction.editReply({ content: "You need to input a number between 1 and 100.", ephemeral: true });
        ...
        }).catch(interaction.editReply({ content: "The bot has insufficient permissions in this channel.", ephemeral: true }));
    },
};