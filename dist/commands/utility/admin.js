    ...
    async execute(interaction) {
        ...
        switch (interaction.options.getSubcommand()) {
            case 'get': {
                ...
                if (entry)
                    return require('./admin/get/autoclear').execute(interaction, entry);
                entry = await models.GuildBan.findOne({ where: { banId: id } });
                if (entry)
                    return require('./admin/get/guildban').execute(interaction, entry);
                entry = await models.Interaction.findOne({ where: { interactionId: id } });
                if (entry)
                    return require('./admin/get/interaction').execute(interaction, entry);
                entry = await models.Server.findOne({ where: { serverId: id } });
                if (entry)
                    return require('./admin/get/server').execute(interaction, entry);
                entry = await models.Server.findAll({ where: { guildId: id, botLeave: null } });
                if (entry)
                    return require('./admin/get/guild').execute(interaction, entry);
                entry = await models.User.findOne({ where: { userId: id } });
                if (entry)
                    return require('./admin/get/user').execute(interaction, entry);
                entry = await models.UserBan.findOne({ where: { banId: id } });
                if (entry)
                    return require('./admin/get/userban.js').execute(interaction, entry);
